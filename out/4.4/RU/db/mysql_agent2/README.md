...
|MySQL|MySQL: Size of database {#DATABASE}|<p>-</p>|ZABBIX_PASSIVE|mysql.db.size["{$MYSQL.DSN}","{#DATABASE}"]<p>**Preprocessing**:</p><p>- DISCARD_UNCHANGED_HEARTBEAT: `1h`</p>|
...
|Zabbix_raw_items|MySQL: Replication Slave status {#MASTER_HOST}|<p>The item gets status information on essential parameters of the slave threads.</p>|ZABBIX_PASSIVE|mysql.replication.get_slave_status["{$MYSQL.DSN}","{#MASTER_HOST}"]|
