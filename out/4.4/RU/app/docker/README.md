...
|Docker|Docker: Name|<p>-</p>|DEPENDENT|docker.name<p>**Preprocessing**:</p><p>- JSONPATH: `$.Name`</p><p>- DISCARD_UNCHANGED_HEARTBEAT: `1d`</p>|
...
|Docker: Failed to fetch info data (or no data for 30m)|<p>Zabbix has not received data for items for the last 30 minutes</p>|`{TEMPLATE_NAME:docker.architecture.nodata(30m)}=1`|WARNING|<p>Manual close: YES</p><p>**Depends on**:</p><p>- Docker: Service is down</p>|
...
|Container {#NAME}: Container has changed the state (new state: {ITEM.VALUE})|<p>Container {#NAME} changed the state. Ack to close.</p>|`{TEMPLATE_NAME:docker.container_info.state.status["{#NAME}"].change()}=1`|INFO||
