...
|{$MYSQL.DSN}|<p>System data source name.</p>|`test`|
...
|Zabbix_raw_items|MySQL: Replication Slave status {#MASTERHOST}|<p>The item gets status information on essential parameters of the slave threads.</p>|ODBC|db.odbc.get[slave_status,"{$MYSQL.DSN}"]<p>**Expression**:</p>`show slave status`|
