...
|Redis|{#SLAVE}: IP address|<p>Slave IP address</p>|DEPENDENT|redis.replication.slave_ip["{#SLAVE}"]<p>**Preprocessing**:</p><p>- JSONPATH: `$.Replication["{#SLAVE}"].ip`</p><p>⛔️ON_FAIL: `CUSTOM_ERROR -> Slave {#SLAVE} is no longer available.`</p><p>- DISCARD_UNCHANGED_HEARTBEAT: `1d`</p>|
|Redis|{#SLAVE}: Port|<p>Slave port number</p>|DEPENDENT|redis.replication.slave_port["{#SLAVE}"]<p>**Preprocessing**:</p><p>- JSONPATH: `$.Replication["{#SLAVE}"].port`</p><p>⛔️ON_FAIL: `CUSTOM_ERROR -> Slave {#SLAVE} is no longer available.`</p>|
