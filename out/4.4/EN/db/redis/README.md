...
|Redis: Total number of connected clients is too high (over {$REDIS.CLIENTS.PRC.MAX.WARN}% in 5m)|<p>-</p>|`{TEMPLATE_NAME:redis.clients.connected.min(5m)}/{Template DB Redis:redis.config.maxclients.last()}*100>{$REDIS.CLIENTS.PRC.MAX.WARN}`|WARNING||
|Redis: Last AOF write operation failed|<p>-</p>|`{TEMPLATE_NAME:redis.persistence.aof_last_write_status.last()}=0`|WARNING||
|Redis: Last RDB save operation failed|<p>-</p>|`{TEMPLATE_NAME:redis.persistence.rdb_last_bgsave_status.last()}=0`|WARNING||
...
|Redis: has been restarted (uptime < 10m)|<p>Uptime is less than 10 minutes</p>|`{TEMPLATE_NAME:redis.server.uptime_in_seconds.last()}<10m`|INFO|<p>Manual close: YES</p>|
