...
|Redis|Redis: Memory used peak|<p>The peak amount of memory used by Redis</p>|DEPENDENT|redis.memory.used_memory_peak<p>**Preprocessing**:</p><p>- JSONPATH: `$.Memory.used_memory_peak`</p>|
...
|Redis|Redis: Loading|<p>Flag indicating a AOF rewrite operation is on-going</p>|DEPENDENT|redis.persistence.aof_rewrite_in_progress<p>**Preprocessing**:</p><p>- JSONPATH: `$.Persistence.aof_rewrite_in_progress`</p>|
...
|Redis|Redis: RDB bgsave in progress|<p>One if a bgsave is in progress and zero otherwise</p>|DEPENDENT|redis.persistence.rdb_bgsave_in_progress<p>**Preprocessing**:</p><p>- JSONPATH: `$.Persistence.rdb_bgsave_in_progress`</p>|
|Redis|Redis: RDB changes since last save|<p>The number of changes since the last background save</p>|DEPENDENT|redis.persistence.rdb_changes_since_last_save<p>**Preprocessing**:</p><p>- JSONPATH: `$.Persistence.rdb_changes_since_last_save`</p>|
...
|Redis|Redis: Connected slaves|<p>The number of connected slaves</p>|DEPENDENT|redis.replication.connected_slaves<p>**Preprocessing**:</p><p>- JSONPATH: `$.Replication.connected_slaves`</p>|
...
|Redis|Redis: Replication backlog history length|<p>The amount of data in the backlog sync buffer</p>|DEPENDENT|redis.replication.repl_backlog_histlen<p>**Preprocessing**:</p><p>- JSONPATH: `$.Replication.repl_backlog_histlen`</p>|
...
|Redis|Redis: Evicted keys|<p>The total number of keys evicted due to the maxmemory limit</p>|DEPENDENT|redis.stats.evicted_keys<p>**Preprocessing**:</p><p>- JSONPATH: `$.Stats.evicted_keys`</p>|
|Redis|Redis: Expired keys|<p>The total number of keys expired from the db</p>|DEPENDENT|redis.stats.expired_keys<p>**Preprocessing**:</p><p>- JSONPATH: `$.Stats.expired_keys`</p>|
...
|Redis|Redis: Instantaneous operations per sec|<p>The number of commands processed by the server per second</p>|DEPENDENT|redis.stats.instantaneous_ops.rate<p>**Preprocessing**:</p><p>- JSONPATH: `$.Stats.instantaneous_ops_per_sec`</p>|
...
|Redis|Redis: Keyspace hits|<p>The total number of successful lookups in the main db</p>|DEPENDENT|redis.stats.keyspace_hits<p>**Preprocessing**:</p><p>- JSONPATH: `$.Stats.keyspace_hits`</p>|
|Redis|Redis: Keyspace misses|<p>The total number of missed lookups in the main db</p>|DEPENDENT|redis.stats.keyspace_misses<p>**Preprocessing**:</p><p>- JSONPATH: `$.Stats.keyspace_misses`</p>|
|Redis|Redis: Latest fork usec|<p>The duration of the latest fork</p>|DEPENDENT|redis.stats.latest_fork_usec<p>**Preprocessing**:</p><p>- JSONPATH: `$.Stats.latest_fork_usec`</p><p>- MULTIPLIER: `1.0E-5`</p>|
...
|Redis|Redis: Pubsub channels|<p>The number of active pubsub channels</p>|DEPENDENT|redis.stats.pubsub_channels<p>**Preprocessing**:</p><p>- JSONPATH: `$.Stats.pubsub_channels`</p>|
|Redis|Redis: Pubsub patterns|<p>The number of active pubsub patterns</p>|DEPENDENT|redis.stats.pubsub_patterns<p>**Preprocessing**:</p><p>- JSONPATH: `$.Stats.pubsub_patterns`</p>|
|Redis|Redis: Rejected connections|<p>The number of rejected connections</p>|DEPENDENT|redis.stats.rejected_connections<p>**Preprocessing**:</p><p>- JSONPATH: `$.Stats.rejected_connections`</p>|
...
|Redis|Redis: Total commands processed|<p>The number of commands processed by the server</p>|DEPENDENT|redis.stats.total_commands_processed<p>**Preprocessing**:</p><p>- JSONPATH: `$.Stats.total_commands_processed`</p>|
...
|Redis|Redis: DB {#DB}: Expires|<p>The number of keys with an expiration</p>|DEPENDENT|redis.db.expires["{#DB}"]<p>**Preprocessing**:</p><p>- JSONPATH: `$.Keyspace["{#DB}"].expires`</p>|
...
|Redis|Redis: Max output buffer|<p>The longest output list among current client connections</p>|DEPENDENT|redis.clients.client_longest_output_list<p>**Preprocessing**:</p><p>- JSONPATH: `$.Clients.client_longest_output_list`</p>|
|Redis|Redis: Max input buffer|<p>The biggest input buffer among current client connections</p>|DEPENDENT|redis.clients.client_biggest_input_buf<p>**Preprocessing**:</p><p>- JSONPATH: `$.Clients.client_biggest_input_buf`</p>|
...
|Redis|Redis: Master replication offset{#SINGLETON}|<p>The replication offset reported by the master</p>|DEPENDENT|redis.replication.master_repl_offset[{#SINGLETON}]<p>**Preprocessing**:</p><p>- JSONPATH: `$.Replication.master_repl_offset`</p>|
|Redis|Redis: Replication second offset{#SINGLETON}|<p>The offset up to which replication IDs are accepted</p>|DEPENDENT|redis.replication.second_repl_offset[{#SINGLETON}]<p>**Preprocessing**:</p><p>- JSONPATH: `$.Replication.second_repl_offset`</p>|
