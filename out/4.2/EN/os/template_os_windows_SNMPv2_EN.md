...
# Template OS Windows memory by SNMPv2
...
|Lack of available memory ( < {$MEMORY.AVAILABLE.MIN} of {ITEM.VALUE2})|<p>Last value: {ITEM.LASTVALUE1}.</p>|`{TEMPLATE_NAME:vm.memory.available[snmp].min(5m)}<{$MEMORY.AVAILABLE.MIN} and {Template OS Windows memory by SNMPv2:vm.memory.total[memTotalReal.0].last()}>0`|AVERAGE||
|High swap space usage ( less than {$SWAP.PFREE.MIN.WARN}% free)|<p>Last value: {ITEM.LASTVALUE1}.</p><p>This trigger is ignored, if there is no swap configured</p>|`{TEMPLATE_NAME:system.swap.pfree[snmp].min(5m)}<{$SWAP.PFREE.MIN.WARN} and {Template OS Windows memory by SNMPv2:system.swap.total[memTotalSwap.0].last()}>0`|WARNING|<p>**Depends on**:</p><p>- High memory utilization ( >{$MEMORY.UTIL.MAX}% for 5m)</p><p>- Lack of available memory ( < {$MEMORY.AVAILABLE.MIN} of {ITEM.VALUE2})</p>|

## Feedback

Please report any issues with the template at https://support.zabbix.com

# Template OS Windows block devices by SNMPv2
...
# Template OS Windows CPU by SNMPv2
...
# Template OS Windows filesystems by SNMPv2
...
Install snmpd agent on Windows OS, enable SNMPv2.  
