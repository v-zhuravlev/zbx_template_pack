...
|----|-----------|----|----|
|CPU core discovery|<p>Discovery of CPU as defined in {$CPU.MATCHES} and {$CPU.NOT_MATCHES}</p>|ZABBIX_PASSIVE|system.cpu.discovery<p>**Filter**:</p>AND <p>- A: {#CPU.NUMBER} MATCHES_REGEX `{$CPU.MATCHES}`</p><p>- B: {#CPU.NUMBER} NOT_MATCHES_REGEX `{$CPU.NOT_MATCHES}`</p>|
...
|CPU|#{#CPU.NUMBER}: CPU utilization|<p>CPU utilization in %</p>|ZABBIX_PASSIVE|system.cpu.util[{#CPU.NUMBER}]|
