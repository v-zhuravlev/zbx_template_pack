...
|Interface {#IFNAME}: Bits received||ZABBIX_PASSIVE|
|Interface {#IFNAME}: Bits sent||ZABBIX_PASSIVE|
|Interface {#IFNAME}: Outbound packets with errors||ZABBIX_PASSIVE|
|Interface {#IFNAME}: Inbound packets with errors||ZABBIX_PASSIVE|
|Interface {#IFNAME}: Outbound packets discarded||ZABBIX_PASSIVE|
|Interface {#IFNAME}: Inbound packets discarded||ZABBIX_PASSIVE|
|Interface {#IFNAME}: Operational status|Indicates the interface RFC2863 operational state as a string.</br>Possible values are:"unknown", "notpresent", "down", "lowerlayerdown", "testing","dormant", "up".</br>Reference: https://www.kernel.org/doc/Documentation/ABI/testing/sysfs-class-net|ZABBIX_PASSIVE|
|Interface {#IFNAME}: Interface type|Indicates the interface protocol type as a decimal value.</br>See include/uapi/linux/if_arp.h for all possible values.</br>Reference: https://www.kernel.org/doc/Documentation/ABI/testing/sysfs-class-net|ZABBIX_PASSIVE|
