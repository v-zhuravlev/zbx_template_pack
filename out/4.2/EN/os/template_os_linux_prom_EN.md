...
|CPU discovery|<p>-</p>|DEPENDENT|system.cpu.discovery[node_exporter]<p>**Preprocessing**:</p><p>- PROMETHEUS_TO_JSON: `{__name__=~"^node_cpu(?:_seconds_total)?$",cpu=~".+",mode="idle"}`</p>|
...
|CPU|Number of CPUs|<p>-</p>|DEPENDENT|system.cpu.num[node_exporter]<p>**Preprocessing**:</p><p>- PROMETHEUS_TO_JSON: `{__name__=~"^node_cpu(?:_seconds_total)?$",cpu=~".+",mode="idle"}`</p><p>- JAVASCRIPT: `//count the number of cores return JSON.parse(value).length `</p><p>- DISCARD_UNCHANGED_HEARTBEAT: `1d`</p>|
...
|CPU|#{#CPU.NUMBER}: CPU utilization|<p>CPU utilization in %</p>|DEPENDENT|system.cpu.util[node_exporter,"{#CPU.NUMBER}"]<p>**Preprocessing**:</p><p>- JAVASCRIPT: `//Calculate utilization return (100 - value)`</p>|
|CPU|#{#CPU.NUMBER}: CPU idle time|<p>The time the CPU has spent doing nothing.</p>|DEPENDENT|system.cpu.idle[node_exporter,"{#CPU.NUMBER}"]<p>**Preprocessing**:</p><p>- PROMETHEUS_PATTERN: `{__name__=~"^node_cpu(?:_seconds_total)?$",cpu="{#CPU.NUMBER}",mode="idle"} `</p><p>- CHANGE_PER_SECOND<p>- MULTIPLIER: `100`</p>|
|CPU|#{#CPU.NUMBER}: CPU system time|<p>The time the CPU has spent running the kernel and its processes.</p>|DEPENDENT|system.cpu.system[node_exporter,"{#CPU.NUMBER}"]<p>**Preprocessing**:</p><p>- PROMETHEUS_PATTERN: `{__name__=~"^node_cpu(?:_seconds_total)?$",cpu="{#CPU.NUMBER}",mode="system"} `</p><p>- CHANGE_PER_SECOND<p>- MULTIPLIER: `100`</p>|
|CPU|#{#CPU.NUMBER}: CPU user time|<p>The time the CPU has spent running users' processes that are not niced.</p>|DEPENDENT|system.cpu.user[node_exporter,"{#CPU.NUMBER}"]<p>**Preprocessing**:</p><p>- PROMETHEUS_PATTERN: `{__name__=~"^node_cpu(?:_seconds_total)?$",cpu="{#CPU.NUMBER}",mode="user"} `</p><p>- CHANGE_PER_SECOND<p>- MULTIPLIER: `100`</p>|
|CPU|#{#CPU.NUMBER}: CPU steal time|<p>The amount of CPU 'stolen' from this virtual machine by the hypervisor for other tasks (such as running another virtual machine).</p>|DEPENDENT|system.cpu.steal[node_exporter,"{#CPU.NUMBER}"]<p>**Preprocessing**:</p><p>- PROMETHEUS_PATTERN: `{__name__=~"^node_cpu(?:_seconds_total)?$",cpu="{#CPU.NUMBER}",mode="steal"} `</p><p>- CHANGE_PER_SECOND<p>- MULTIPLIER: `100`</p>|
|CPU|#{#CPU.NUMBER}: CPU softirq time|<p>The amount of time the CPU has been servicing software interrupts.</p>|DEPENDENT|system.cpu.softirq[node_exporter,"{#CPU.NUMBER}"]<p>**Preprocessing**:</p><p>- PROMETHEUS_PATTERN: `{__name__=~"^node_cpu(?:_seconds_total)?$",cpu="{#CPU.NUMBER}",mode="softirq"} `</p><p>- CHANGE_PER_SECOND<p>- MULTIPLIER: `100`</p>|
|CPU|#{#CPU.NUMBER}: CPU nice time|<p>The time the CPU has spent running users' processes that have been niced.</p>|DEPENDENT|system.cpu.nice[node_exporter,"{#CPU.NUMBER}"]<p>**Preprocessing**:</p><p>- PROMETHEUS_PATTERN: `{__name__=~"^node_cpu(?:_seconds_total)?$",cpu="{#CPU.NUMBER}",mode="nice"} `</p><p>- CHANGE_PER_SECOND<p>- MULTIPLIER: `100`</p>|
|CPU|#{#CPU.NUMBER}: CPU iowait time|<p>Amount of time the CPU has been waiting for I/O to complete.</p>|DEPENDENT|system.cpu.iowait[node_exporter,"{#CPU.NUMBER}"]<p>**Preprocessing**:</p><p>- PROMETHEUS_PATTERN: `{__name__=~"^node_cpu(?:_seconds_total)?$",cpu="{#CPU.NUMBER}",mode="iowait"} `</p><p>- CHANGE_PER_SECOND<p>- MULTIPLIER: `100`</p>|
|CPU|#{#CPU.NUMBER}: CPU interrupt time|<p>The amount of time the CPU has been servicing hardware interrupts.</p>|DEPENDENT|system.cpu.interrupt[node_exporter,"{#CPU.NUMBER}"]<p>**Preprocessing**:</p><p>- PROMETHEUS_PATTERN: `{__name__=~"^node_cpu(?:_seconds_total)?$",cpu="{#CPU.NUMBER}",mode="irq"} `</p><p>- CHANGE_PER_SECOND<p>- MULTIPLIER: `100`</p>|
|CPU|#{#CPU.NUMBER}: CPU guest time|<p>Guest  time (time  spent  running  a  virtual  CPU  for  a  guest  operating  system)</p>|DEPENDENT|system.cpu.guest[node_exporter,"{#CPU.NUMBER}"]<p>**Preprocessing**:</p><p>- PROMETHEUS_PATTERN: `{__name__=~"^node_cpu(?:_guest_seconds_total)?$",cpu="{#CPU.NUMBER}",mode=~"^(?:user|guest)$"} `</p><p>- CHANGE_PER_SECOND<p>- MULTIPLIER: `100`</p>|
|CPU|#{#CPU.NUMBER}: CPU guest nice time|<p>Time spent running a niced guest (virtual CPU for guest operating systems under the control of the Linux kernel)</p>|DEPENDENT|system.cpu.guest_nice[node_exporter,"{#CPU.NUMBER}"]<p>**Preprocessing**:</p><p>- PROMETHEUS_PATTERN: `{__name__=~"^node_cpu(?:_guest_seconds_total)?$",cpu="{#CPU.NUMBER}",mode=~"^(?:nice|guest_nice)$"} `</p><p>- CHANGE_PER_SECOND<p>- MULTIPLIER: `100`</p>|
...
|Storage|{#FSNAME}: Space utilization|<p>Space utilization in % for {#FSNAME}</p>|CALCULATED|vfs.fs.pused[node_exporter,"{#FSNAME}"]<p>**Expression**:</p>`(last(vfs.fs.used[node_exporter,"{#FSNAME}"])/(last(vfs.fs.free[node_exporter,"{#FSNAME}"])+last(vfs.fs.used[node_exporter,"{#FSNAME}"])))*100`|
