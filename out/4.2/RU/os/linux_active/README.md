...
|High swap space usage ( less than {$SWAP.PFREE.MIN.WARN}% free)|<p>This trigger is ignored, if there is no swap configured</p>|`{TEMPLATE_NAME:system.swap.size[,pfree].min(5m)}<{$SWAP.PFREE.MIN.WARN} and {Template Module Linux memory by Zabbix agent active:system.swap.size[,total].last()}>0`|WARNING|<p>**Depends on**:</p><p>- Lack of available memory ( < {$MEMORY.AVAILABLE.MIN} of {ITEM.VALUE2})</p><p>- Мало свободной памяти ОЗУ ( >{$MEMORY.UTIL.MAX}% for 5m)</p>|
...
|Group|Name|Description|Type|Key and additional info|
|-----|----|-----------|----|---------------------|
|Network_interfaces|Interface {#IFNAME}: Бит получено||ZABBIX_ACTIVE|net.if.in["{#IFNAME}"]<p>**Preprocessing**:</p><p>- CHANGE_PER_SECOND<p>- MULTIPLIER: `8`</p>|
|Network_interfaces|Interface {#IFNAME}: Бит отправлено||ZABBIX_ACTIVE|net.if.out["{#IFNAME}"]<p>**Preprocessing**:</p><p>- CHANGE_PER_SECOND<p>- MULTIPLIER: `8`</p>|
|Network_interfaces|Interface {#IFNAME}: Исходящих пакетов с ошибками||ZABBIX_ACTIVE|net.if.out["{#IFNAME}",errors]<p>**Preprocessing**:</p><p>- CHANGE_PER_SECOND|
|Network_interfaces|Interface {#IFNAME}: Входящих пакетов с ошибками||ZABBIX_ACTIVE|net.if.in["{#IFNAME}",errors]<p>**Preprocessing**:</p><p>- CHANGE_PER_SECOND|
|Network_interfaces|Interface {#IFNAME}: Пакетов отброшено (исходящих)||ZABBIX_ACTIVE|net.if.out["{#IFNAME}",dropped]<p>**Preprocessing**:</p><p>- CHANGE_PER_SECOND|
|Network_interfaces|Interface {#IFNAME}: Пакетов отброшено (входящих)||ZABBIX_ACTIVE|net.if.in["{#IFNAME}",dropped]<p>**Preprocessing**:</p><p>- CHANGE_PER_SECOND|
...
|Group|Name|Description|Type|Key and additional info|
|-----|----|-----------|----|---------------------|
|General|Время загрузки системы|<p>-</p>|ZABBIX_ACTIVE|system.boottime<p>**Preprocessing**:</p><p>- DISCARD_UNCHANGED_HEARTBEAT: `1h`</p>|
|General|Локальное время системы|<p>System local time of the host.</p>|ZABBIX_ACTIVE|system.localtime|
|General|Имя системы|<p>System host name.</p>|ZABBIX_ACTIVE|system.hostname<p>**Preprocessing**:</p><p>- DISCARD_UNCHANGED_HEARTBEAT: `1h`</p>|
|General|Описание системы|<p>The information as normally returned by 'uname -a'.</p>|ZABBIX_ACTIVE|system.uname<p>**Preprocessing**:</p><p>- DISCARD_UNCHANGED_HEARTBEAT: `1d`</p>|
